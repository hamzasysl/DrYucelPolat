    ...
    <div class="bg-white border-b border-ink-100">
        <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-2.5 flex flex-wrap items-center gap-3">
            ...
            <div class="flex flex-wrap items-center gap-x-4 gap-y-1.5 lg:gap-x-4 text-[13px] text-ink-500 flex-1 min-w-0">
                {{-- Telefon → leaf yeşil. Mobilde tek satırda numara da görünür. --}}
                <a href="tel:+900000000000"
                   class="group inline-flex items-center gap-2 pr-3 lg:pr-4 lg:border-r border-leaf-500/55 hover:text-leaf-500 transition-colors">
                    ...
                    <span class="font-light text-[12.5px] sm:text-[13px]">+90 (000) 000 00 00</span>
                </a>

                {{-- Email → leaf yeşil. Mobilde gizli (sm+ üzeri ikon, md+ üzeri text). --}}
                <a href="mailto:info@dryucelpolat.com"
                   class="group hidden sm:inline-flex items-center gap-2 pr-3 lg:pr-4 lg:border-r border-leaf-500/55 hover:text-leaf-500 transition-colors">
                    ...
                    <span class="font-light hidden md:inline">info@dryucelpolat.com</span>
                </a>

                {{-- Adres → leaf yeşil pin. Mobilde gizli. --}}
                <div class="group hidden sm:inline-flex items-center gap-2">
                    ...
                    <span class="font-light hidden lg:inline">Klinik Adresi, Sarıyer / İstanbul</span>
                </div>
            </div>

            {{-- Social: yalnız IG / FB / YT --}}
            <div class="flex items-center gap-1.5">
                <a href="#" class="social-pill" aria-label="Instagram"><i class="fab fa-instagram"></i></a>
                <a href="#" class="social-pill" aria-label="Facebook"><i class="fab fa-facebook-f"></i></a>
                <a href="#" class="social-pill" aria-label="YouTube"><i class="fab fa-youtube"></i></a>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white">
        <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-3 flex items-center gap-6 relative"
            ...
            <a href="{{ route('home') }}" class="flex-shrink-0 inline-block">
                <img src="{{ asset('img/logo.png') }}" alt="{{ config('app.name') }}" class="h-[52px] lg:h-[60px] w-auto">
            </a>
                    ...
                    @if (!empty($item['dropdown']))
                        ...
                        <div class="relative"
                            ...
                            <div x-show="dropOpen" x-cloak
                                ...
                                <div class="relative bg-white rounded-2xl shadow-[0_20px_50px_-12px_rgba(15,61,90,0.25)] border border-ink-100 overflow-hidden">
                                    {{-- Üst aksan şeridi --}}
                                        ...
                                        <div class="grid grid-cols-2 gap-x-2 gap-y-1">
                                            @foreach ($treatments as $i => $t)
                                                @php
                                                    ...
                                                    $isActive = $current === 'services.show' && $currentSlug === $t['slug'];
                                                    $href = $hasPage ? route('services.show', $t['slug']) : '#';
                                                @endphp
                                                <a href="{{ $href }}"
                                                   ...
                                                   class="group flex items-center gap-2 min-[1200px]:gap-3 p-1.5 min-[1200px]:p-2.5 rounded-lg transition-colors
                                                    ...
                                                    <span class="w-7 h-7 min-[1200px]:w-9 min-[1200px]:h-9 rounded-lg {{ $c['bg'] }} {{ $c['text'] }} inline-flex items-center justify-center shrink-0 transition-colors {{ $c['fill'] }} group-hover:text-white">
                                                        <i class="fas {{ $t['icon'] }} text-[10px] min-[1200px]:text-sm"></i>
                                                    </span>
            ...
            <a href="{{ $whatsapp }}" target="_blank" rel="noopener"
               ...
               aria-label="WhatsApp ile randevu al">
                ...
                <span class="relative bg-gradient-to-b from-deep-700 to-deep-400
                             ...
                             shadow-[0_2px_8px_rgba(15,61,90,0.15)]
                             group-hover:shadow-[0_10px_28px_rgba(30,95,158,0.32)]
                             transition-all duration-300
            ...
            <div x-show="open" x-cloak
                 ...
                 class="mob-menu absolute top-full left-0 right-0 bg-white lg:hidden overflow-hidden z-50
                        shadow-[0_24px_50px_-16px_rgba(15,61,90,0.30)] rounded-b-2xl"
                 style="display:none">
                            ...
                            <div x-data="{ mobOpen: false }" class="mob-nav-item" style="--i: {{ $idx }};">
                                ...
                                <div x-show="mobOpen" x-cloak style="display:none"
                                    ...
                                    @foreach ($treatments as $t)
                                        @php
                                            $mobActive = $current === 'services.show' && request()->route('slug') === $t['slug'];
                                            $mobHref = ($t['has_page'] ?? false) ? route('services.show', $t['slug']) : '#';
                                        @endphp
                                        <a href="{{ $mobHref }}"
                                           ...
                                           class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] font-medium transition-colors text-ink-700
                                            ...
                                            <i class="fas {{ $t['icon'] }} text-deep-400 text-xs w-4 text-center"></i>
                                            {{ $t['title'] }}
                                        </a>
                                    @endforeach
                                </div>
